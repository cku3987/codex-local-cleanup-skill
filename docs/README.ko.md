...
- 모바일에만 계속 보이는 항목이 현재 활성 DB에는 없으면 legacy `.codex/sqlite/state_*.sqlite`와 `.codex/sqlite/logs_*.sqlite`도 확인합니다.
- 데스크탑에서 이름만 바뀐 저장 프로젝트는 삭제 대상이 아니라 표시명 동기화 문제로 취급합니다.
