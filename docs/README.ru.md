...
- Если мобильный Codex все еще показывает записи, которых уже нет в активной базе данных, проверяет legacy `.codex/sqlite/state_*.sqlite` и `.codex/sqlite/logs_*.sqlite`.
- Считает переименованные сохраненные проекты проблемой синхронизации отображаемого имени, а не устаревшей целью для удаления.
