...
- モバイル側にだけ残って見える項目が現在のアクティブ DB に存在しない場合は、legacy の `.codex/sqlite/state_*.sqlite` と `.codex/sqlite/logs_*.sqlite` も確認します。
- Desktop で名前だけ変更された保存済みプロジェクトは、削除対象ではなく表示名同期の問題として扱います。
